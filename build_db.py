...
def main():
    ...
    with open(flightSampleCsv,"r",encoding="utf-8") as f:
        ...
        for line in reader:
            ...
            conn.execute("INSERT INTO Vol (firstseen, takeofftime, lastseen, landingtime, callsign, estdepartureairport, airportransFloatdeparture, estarrivalairport, airportransFloatdestination, registration) VALUES (?,?,?,?,?,?,?,?,?,?)",
                         (transFloat(row.get("firstseen")),transFloat(row.get("takeofftime")), 
                          ...
                          clean(row.get("estdepartureairport")),clean(row.get("airportransFloatdeparture")),
                          clean(row.get("estarrivalairport")),clean(row.get("airportransFloatdestination")),reg))
    conn.commit()
